...
export function runForceGraphPixi(
  ...
  function ticked() {
    ...
    renderData.links.forEach((link) => {
      ...
      linksGfx.lineStyle({
        width: Math.sqrt(link.value ?? 1),
        color: 0x999999
      });
      linksGfx.moveTo(source.x, source.y);
      linksGfx.lineTo(target.x, target.y);
      linksGfx.interactive = true;
      linksGfx.buttonMode = true;
      linksGfx.on('mouseover', () => {
        linksGfx.lineStyle({
          width: Math.sqrt(5),
          color: 0x999999
        });
  ...
  function dragable() {
    d3.select(app.view)
      .call(d3.drag()
        .container(app.view)
        .subject((event) => simulation.find(event.x, event.y, 10))
